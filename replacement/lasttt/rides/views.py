...
from django.views.generic import ListView, DetailView, CreateView, UpdateView, DeleteView
from django.urls import reverse_lazy
# ... (ensure you have all necessary imports, including geopy)
from geopy.distance import great_circle
from .models import Ride, RideEvent # Ensure RideEvent is imported
# ... (other imports)

# ⚠️ NOTE: These coordinates are approximate and should be verified for accuracy!
LOCATION_COORDINATES = {
    'Clark Main Gate': (15.1764, 120.5519),
    'SM City Clark': (15.1795, 120.5753),
    'Clark Parade Grounds': (15.1782, 120.5658),
    'Widus Hotel & Casino': (15.1873, 120.5770),
    'Marquee Mall': (15.1664, 120.5898),
    'Clark Museum': (15.1782, 120.5590),
    'Aqua Planet': (15.1979, 120.5552),
    'Clark International Airport': (15.1866, 120.5539),
    'Clark Development Corporation': (15.1760, 120.5570),
    'Fontana Leisure Park': (15.1685, 120.5483),
    'Clark Sun Valley': (15.2201, 120.5650),
    'Midori Clark Hotel': (15.1869, 120.5768),
    'Royce Hotel & Casino': (15.1875, 120.5772),
}
...
class CustomerBookRideView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        # Assuming LOCATION_CHOICES is defined in your Ride model
        context['locations'] = self.model.LOCATION_CHOICES
        return context
    ...
    def form_valid(self, form):
        ...
        # 2. INITIALIZE INSTANCE FIELDS (commit=False equivalent)
        form.instance.customer = self.request.user
        form.instance.status = 'PENDING'

        # 3. DISTANCE CALCULATION (Fix for IntegrityError)
        pickup_loc = LOCATION_COORDINATES.get(form.cleaned_data['pickup'])
        dest_loc = LOCATION_COORDINATES.get(form.cleaned_data['destination'])

        if pickup_loc and dest_loc:
            distance_km = great_circle(pickup_loc, dest_loc).km
            form.instance.total_distance = round(distance_km, 2)
        else:
            # Fallback to 0.0 if coordinates are missing (safety net)
            form.instance.total_distance = 0.0
            messages.warning(self.request, 'Could not calculate distance. Defaulting to 0km.')

        # 4. SAVE THE RIDE OBJECT TO DATABASE (Critical step!)
        response = super().form_valid(form)

        # 5. CREATE INITIAL RIDE EVENT (Based on your previous logic)
        RideEvent.objects.create(
            ride=self.object,
            step=1,  # Ride Requested
            description=f"Ride requested by {self.request.user.get_full_name()}"
